...
//filefolderlistcontent.jsp updates g_addReqPageMenuBar based on iportal's web.xml
var g_addReqPageMenuBar = false;
var g_viewXlsInRequester = false;
	...
	if (  newWidth != '' &&  newHeight != '' )
	{
		...
		else 
		{
			 if( isIE() )
			 {
				 ...
				 newTop = outerMostParentIframe.screenTop + ((outerMostParentIframe.document.body.offsetHeight - newHeight) /2);
			 }
			 else
		...
		if (window.outerWidth) 
		{
		 ...
		 else 
		 {
			 ...
			 {
				 var outerMostParentIframe = getOutermostiFrame();
				 newLeft = outerMostParentIframe.screenLeft + ((outerMostParentIframe.document.body.offsetWidth - newWidth) /2);
			 }else
			 {
...
function openModalPopupWindow(url, windowName, newWidth, newHeight)
{
	if (window.showModalDialog)
	{
		// For IE browsers.
		window.showModalDialog(url, windowName, 'dialogWidth:' + newWidth + 'px;dialogHeight:' + newHeight + 'px');
	}
	else 
		openPopupWindow(url, windowName, newWidth, newHeight, "modal=yes");
}
...
//caller can use optionalParam to decide how to customize requester page window
function openRequesterPageWindow(url, windowName, optionalParam, newWindow)
{
	if( newWindow )
	{
	...
	else
	{
		...
		var width = 1200;
		// add menu bar
		if ( g_addReqPageMenuBar == true )
		{
			spec += " ,menubar=yes";
		}	
		if ( optionalParam )
		{
			// customize requester page window spec here such as not adding the menu bar.
		}
		url += "&__from=requestPage";
		openPopupWindow(url, windowName,width,height, spec );
	}
}
